...
const ClientesWeb = ({ route }) => {
  ...
  useFocusEffect(
    React.useCallback(() => {
      ...
      try {
          ...
          // Cargar los clientes desde IndexedDB
        const clientesFromDB = await indexedDBHandler.obtenerClientes();
        setClientes(clientesFromDB);
          console.log(`Se cargaron ${clientesFromDB.length} clientes desde IndexedDB`);
  ...
  const handleClientClick = async (cliente) => {
    ...
    let preventaNumero = await nextPreventa(); 
      let edit = false;
    navigation.navigate('PreventaWeb', { preventaNumero, cliente, edit });
    } catch (error) {
